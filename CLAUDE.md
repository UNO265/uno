...
- **CASE #003부터는 `docs/KANENAZO_PERFORMANCE_v1.md`(성과 개선 지침)도 함께 적용한다.** 우선순위는 TOPIC → TITLE → THUMBNAIL → 첫 5~30초 → STORY → VISUAL → 세부 애니메이션이다. 제목은 정해진 틀이 아니라 SEO(검색 표기·핵심 검색어를 앞에)·짧게(20자 안팎)·후킹(의문·대립·역설·의외의 숫자 중 하나)을 동시에 만족하게 만들고(성과 지침 4-1), vidIQ 같은 제목 추천·점수는 검색어 후보로만 쓰고 A/B 테스트로 정하고(4-2), 썸네일은 [물체 하나 + 숫자 하나 + 짧은 질문]으로 설계하고, 결론은 답을 먼저 말한 뒤 이유를 설명한다. **Shorts는 `docs/KANENAZO_SHORTS_v1.md`(쇼츠 지침서)를 따른다**: 목적은 궁금하게 만들어 롱폼으로 보내는 것, 한 편 = 작은 답 + 본편의 질문(본편 제목·썸네일 질문과 같은 말), 마지막 2초는 본편 썸네일과 같은 물체 + 질문(반복 재생 연출 없음), **롱폼에 집중하고 쇼츠는 CASE마다 입구 0~1편 + 필요할 때 실험(주제 후보 검증 등)만, 제작 시간은 CASE의 15% 이내**(関連動画 클릭 편당 20회 이상 또는 본편의 Shorts 피드 5% 이상이면 2편으로 늘리고, 두 CASE 연속 미달이면 입구 쇼츠 중단). 채널의 주력은 롱폼이며 Shorts의 목적은 본편 유입이다(1차 지표: 関連動画 클릭·본편의 「Shorts 피드」 유입, 본편을 먼저 공개하고 Shorts에 関連動画 지정). Shorts는 롱폼의 축약판이 아니라 별도 콘텐츠로 설계한다(첫 프레임: 큰 제품 + 큰 숫자 + 짧은 질문, 한 편에 한 요소만 바꿔 실험). 업로드 전에는 `docs/templates/upload-checklist.md`를 확인하고, 업로드 후 데이터는 `docs/performance-log.md`에 기록한다.
