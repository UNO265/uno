...
- CASE #003부터 REAL 컷은 유료 스톡 대신 **AI 생성 실사 영상**(ChatGPT·Gemini)을 쓴다. AI 영상은 현실의 기록이 아니므로 EVIDENCE 라벨을 붙이지 않고 「イメージ」를 표시한다. 브랜드·로고·실존 인물·화면 속 글자는 넣지 않고, 글자와 숫자는 Remotion에서 얹는다. 업로드할 때 YouTube의 합성 콘텐츠 공개를 「はい」로 한다. 프롬프트 예: `docs/case003-ai-footage-prompts.md`.
