...
## 최종 완성본 점검(`video/out/case003_final.mp4`, 도입부 재배치 후)

- **길이**: 13:47.8(1920×1080, 30fps, H.264 / AAC)
- **음량**: 통합 -14.1 LUFS, True Peak -1.1 dBFS, LRA 2.8 LU
- **클리핑**: 0 샘플
- **1.2초 이상의 무음 구간**: 없음(타이틀 직후의 1.5초 공백은 BGM 구간 c3_02를 타이틀(M06)부터 시작하게 해서 없앰)
- **전달용 분할**: `video/out/case003_parts/`의 3개 파일(각 30MB 미만, 키프레임 기준 분할) + `join_windows.bat` / `join_mac.command`(ffmpeg로 무손실 합치기, 합친 길이 13:47.8 확인)
