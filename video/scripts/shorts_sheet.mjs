...
//         CASE=002 node scripts/shorts_sheet.mjs 1   → out/shorts_sheet_002_1.jpg
...
const cs = process.env.CASE ?? "001";
const tag = cs === "001" ? n : `${cs}_${n}`;
const tl = JSON.parse(fs.readFileSync(path.join(root, `public/case${cs}_shorts/short${n}/timeline.json`), "utf8"));
const out = path.join(root, `out/shorts_stills_${tag}`);
fs.mkdirSync(out, { recursive: true });
const serveUrl = await bundle({ entryPoint: path.join(root, "src/index.ts") });
const browser = await openBrowser("chrome", { browserExecutable: process.env.REMOTION_CHROME ?? null });
const composition = await selectComposition({ serveUrl, id: `Short${cs}-${n}`, puppeteerInstance: browser });
...
execSync(`ffmpeg -v error -y ${inputs} -filter_complex "xstack=inputs=${files.length}:layout=${pos}:fill=black" -frames:v 1 ../shorts_sheet_${tag}.jpg`, { cwd: out });
console.log(`out/shorts_sheet_${tag}.jpg`);
