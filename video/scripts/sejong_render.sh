...
for ((s = 0; s < TOTAL; s += CHUNK)); do
  ...
  npx remotion render src/sejong/index.ts Sejong "$f.tmp.mp4" --frames=$s-$e --muted --concurrency=4 --crf=20 $BROWSER --log=error
  mv "$f.tmp.mp4" "$f"
done
[[ -s "$PARTS/audio.wav" ]] || npx remotion render src/sejong/index.ts Sejong "$PARTS/audio.wav" --codec=wav $BROWSER --log=error
